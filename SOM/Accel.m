...
% Kep2   = Element(const.GM_Earth, Y);
% N      = sqrt(const.GM_Earth/(Kep2(1)^3));
% N      = N;
% ap     = (const.GM_Earth/(N^2))^(1/3);
% Kep2(1)= ap;
...
if (AuxParam.drag)
    ...
    %   dens = Density_HP(MJD_TT, T*Y(1:3));
    a = a + AccelDrag(dens,Y(1:3),Y(4:6),T,Omega,AuxParam.Bstar);
    
end
    
    %Y(4:6) = Y(4:6)*(sqrt(const.GM_Earth*ap)/norm(Y(1:3))/norm(Y(4:6)))
    %Y(4:6) = Y(4:6)*(1-AuxParam.TD1/norm(Y(4:6)));
    %a(1:3) = a(1:3)*(1-AuxParam.TD2/norm(a(1:3)));
